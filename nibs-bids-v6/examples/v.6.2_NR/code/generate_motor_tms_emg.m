...
emgDir = [subDir,filesep,'emg'];
mkdir(emgDir);

nibsDir = [subDir,filesep,'nibs'];
mkdir(nibsDir);
...
% Event index
event_index = 1:length(onset);
event_index = event_index';

% Write table
T = table(onset,duration,trial_type,event_index, 'VariableNames', {'onset','duration','event_id','event_index'});
outputName = [filePath,id,filesep,'emg',filesep,id,'_',taskname,'_events.tsv'];
writetable(T, outputName, 'Delimiter', 'tab', 'FileType', 'text');
...
data.event_id.Description = 'Single and paired pulses for SICI protocol.';
data.event_id.Levels.single = 'Single pulse test stimulus';
data.event_id.Levels.paired = 'Paired pulse test stimulus; ISI = 2ms, conditioning pulse intensity = 80% AMT';
data.event_index.LongName = 'Index of events';
data.event_index.Description = 'Event number for linking with markers.tsv';
...
% _nibs.tsv
event_id = {'single';'paired'};
target_id = {'left_M1';'left_M1'};
coil_id = {'coil1';'coil1'};
pulse_shape = {'Monophasic';'Monophasic'};
pulse_duration = {200;200};
pulses_in_event = {1;2};
pulses_in_train = {1;2};
pulse_repetition_interval = {'n/a';3};
pulse_intensity_mso = {50;50};
rmt_intensity_mso = {40;40};
amt_intensity_mso = {35;35};
stim_id = {'n/a';'stim1'};
...
data.ManufacturerModelName = 'BiStim^2';
data.ManufacturerSerialNumber = '3234-00';
data.CoilSet.ModelName = 'D70';
data.CoilSet.SerialNumber = '4150-00';
