...
#### `StimulationSystemType` 

When the `stimsys-<label>` entity is used in a `*_nibs.*` filename, the corresponding `*_nibs.json` sidecar MUST include `StimulationSystemType`.

- **Field name:** `StimulationSystemType`
- **Data type:** string
- **Requirement level:** REQUIRED if `stimsys-<label>` is present; otherwise OPTIONAL.
- **Definition:** Human-readable description of the stimulation system indexed by the `stimsys-<label>` entity.
- **Consistency rule:** For a given dataset, the same `stimsys-<label>` MUST map to a consistent `StimulationSystemType` description across subjects/sessions.
- **Recommended convention:** `StimulationSystemType` SHOULD start with the exact `stimsys` label (filename-friendly), optionally followed by a more descriptive name.

Examples:
- filename entity: `stimsys-tms` → `"StimulationSystemType": "tms (transcranial magnetic stimulation)"`
- filename entity: `stimsys-tes` → `"StimulationSystemType": "tes (transcranial electrical stimulation)"`
...
* Rule for `threshold_pulse_intensity`: 
	...
	(1) If `PulseIntensityScalingVector` is present and `PulseIntensityScalingReference` = `threshold`, then `threshold_pulse_intensity` SHOULD be omitted, since pulse-specific threshold-relative intensities are fully determined by `threshold_reference_intensity` and the scaling vector.	
...
* Interpretation
	...
	- StimID defines the stimulus atom.
...
* Interpretation
 ...
 - StimID = stim_1 defines the stimulus atom.
...
A stimulation instance corresponds to the delivery of a burst-based train.
A train is composed of repeated bursts, and each burst is composed of repeated stimulus atoms referenced by `stim_id`.
A delivered burst-based train can be represented by one row in *_nibs.tsv.

* Stimulus definition (*_nibs.json → StimulusSet)

This example defines a single-pulse stimulus atom, which is repeated within bursts and trains.
...
* Interpretation
	...
	- `StimID` defines the stimulus atom referenced by `stim_id` in *_nibs.tsv.
	- The burst structure is fully defined by:

		- `burst_stimuli_number` (TSV)
		- `burst_stimuli_interval` or `burst_stimuli_rate` (TSV)

	- The train structure (burst repetition) is fully defined by:

		- `train_burst_number` (TSV)
		- `inter_burst_interval` or `train_burst_rate` (TSV)

	- If the train is repeated as a higher-level pattern (e.g., iTBS-style on/off cycles), repetition can be encoded by:

		- `train_number` (TSV)
		- `inter_train_pulse_interval` (TSV)

* Stimulation instances (*_nibs.tsv)

Each row corresponds to one delivered burst-based protocol instance (for example, one iTBS-like session described parametrically).

```
stim_id	stim_count 	base_pulse_intensity 	threshold_type 	threshold_reference_intensity	threshold_pulse_intensity 	burst_stimuli_number 	burst_stimuli_interval	train_burst_number	inter_burst_interval
stim_1	1 			55 						resting_motor 	50 								110 						3 						0.02 					200 				0.2
```

* Timing note

- If bursts/trains are delivered irregularly or task-triggered, onsets SHOULD be recorded in *_events.tsv.
- If periodic, the burst/train timing parameters above are sufficient to reconstruct the intended temporal pattern.
